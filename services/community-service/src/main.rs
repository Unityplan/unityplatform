...
async fn main() -> std::io::Result<()> {
    ...
    let metrics_collector =
        shared_lib::MetricsCollector::new("community_service_service", env!("CARGO_PKG_VERSION"));
    ...
    tracing::info!("👋 Community service stopped gracefully");
    Ok(())
}
...
#[utoipa::path(
    ...
    responses(
        (status = 200, description = "Prometheus metrics", 
            ...
            body = String,
            example = "# HELP community_service_service_http_requests_total Total HTTP requests"
        )
    )
)]
async fn metrics(
