...
async fn test_get_followers() {
    ...
    service
        .follow_user(follower1_id, user_id)
        .await
        .expect("Follow should succeed");
    service
        .follow_user(follower2_id, user_id)
        .await
        .expect("Follow should succeed");
...
async fn test_get_following() {
    ...
    let following1_id = ctx
        .create_user("following1", "following1@example.com")
        .await;
    let following2_id = ctx
        .create_user("following2", "following2@example.com")
        .await;

    service
        .follow_user(user_id, following1_id)
        .await
        .expect("Follow should succeed");
    service
        .follow_user(user_id, following2_id)
        .await
        .expect("Follow should succeed");
