...
async fn test_block_user() {
    ...
    service
        .block_user(blocker_id, blocked_id, Some("Spam".to_string()))
        .await
        .expect("Block should succeed");

    // Verify block exists
    let blocked_users = service
        .get_blocked_users(blocker_id)
        .await
...
async fn test_unblock_user() {
    ...
    // Block user
    service
        .block_user(blocker_id, blocked_id, Some("Testing".to_string()))
        .await
        .expect("Block should succeed");
...
async fn test_blocking_removes_connections() {
    ...
    let user1_following = service
        .get_following(user1_id)
        .await
        .expect("Query should succeed");
    assert_eq!(
        user1_following.len(),
        1,
        "Should have one following before block"
    );

    // User1 blocks User2
    service
        .block_user(user1_id, user2_id, Some("Not interested".to_string()))
        .await
        .expect("Block should succeed");

    // Verify connections are removed
    let user1_following_after = service
        .get_following(user1_id)
        .await
        .expect("Query should succeed");
    let user2_following_after = service
        .get_following(user2_id)
        .await
        .expect("Query should succeed");

    assert_eq!(
        user1_following_after.len(),
        0,
        "Blocker's following should be empty"
    );
    assert_eq!(
        user2_following_after.len(),
        0,
        "Blocked user's following should be empty"
    );
...
async fn test_get_blocked_users() {
    ...
    service
        .block_user(blocker_id, blocked1_id, Some("Reason 1".to_string()))
        .await
        .expect("Block should succeed");
    service
        .block_user(blocker_id, blocked2_id, Some("Reason 2".to_string()))
        .await
        .expect("Block should succeed");
...
async fn test_mutual_blocks() {
    ...
    service
        .block_user(user1_id, user2_id, Some("Mutual dislike".to_string()))
        .await
        .expect("Block should succeed");
    service
        .block_user(user2_id, user1_id, Some("Mutual dislike".to_string()))
        .await
        .expect("Block should succeed");

    // Verify both blocks exist
    let user1_blocked = service
        .get_blocked_users(user1_id)
        .await
        .expect("Query should succeed");
    let user2_blocked = service
        .get_blocked_users(user2_id)
        .await
        .expect("Query should succeed");
...
async fn test_update_block_reason() {
    ...
    // Initial block
    service
        .block_user(blocker_id, blocked_id, Some("Initial reason".to_string()))
        .await
        .expect("Block should succeed");

    // Update reason by blocking again (ON CONFLICT DO UPDATE)
    service
        .block_user(blocker_id, blocked_id, Some("Updated reason".to_string()))
        .await
...
async fn test_block_prevents_follow() {
    ...
    service
        .block_user(
            blocker_id,
            blocked_id,
            Some("Don't want to interact".to_string()),
        )
        .await
        .expect("Block should succeed");
    ...
    assert!(
        ...
        "Following a blocked user should fail"
    );
    ...
    assert!(
        reverse_follow_result.is_err(),
        "Blocked user should not be able to follow blocker"
    );
